...
func(ag *AutoGenerated ) CalculateLine (gears [][]int,ScoreCfgMap map[int][]int,conf *readconf.ConfigFile,prizesummap map[int]map[int]int ) int{
	//gears := ag.Base()
...
func(ag *AutoGenerated ) FreeCalculateLine (gears [][]int,ScoreCfgMap map[int][]int,conf *readconf.ConfigFile,prizesummap map[int]map[int]int ) int{
	//gears := ag.Base()
	//gears := TestBase() //测试用的轮子，方便测试
	beforeicon := 0
	icon := 0
	...
	sum := 0
	//var prizemap map[int]int
	for i:=conf.FreeAssignline[0];i<=conf.FreeAssignline[1];i++{
		count := 0
		prizemap := make(map[int]int)
		for ii,jj := range ag.Lines[i]{
			icon = gears[ii][jj]
