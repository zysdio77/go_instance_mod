package handlelogic

import (
	"fmt"
	"nightclub/readconf"
)

func (ag *AutoGenerated	)Score(ScoreCfgMap map[int][]int,prizemap  map[int]int,conf *readconf.ConfigFile,prizesummap map[int]map[int]int )  int{
	maxscore := 0
	for k,v := range prizemap{
		if k ==21 || k == 22 {
			continue
		}
		prizesummap[v][k]++
		//prizenummap.StatisticsPrizeSumMap(prizemap)
		score :=ScoreCfgMap[k][v-1]
		if score >= maxscore{
			maxscore = score
		}
		if conf.Debug{
			fmt.Printf("key : %v value : %v ,scorecfg : %v ,score : %v\n",k,v,ScoreCfgMap[k],score)
		}
	}
	if conf.Debug{
		fmt.Printf("maxscore:%v\n",maxscore)
	}
	return maxscore

}

func (ag *AutoGenerated	)RespinScore(bonusscoreslist []int) int {
	respinscores := 0
		for _ , j := range bonusscoreslist	{
			respinscore :=ag.ScoreBase*j
			respinscores = respinscores + respinscore
			//fmt.Println("respingscore: ",ag.ScoreBase*j)
		}
	//fmt.Println("respingscores: ",respinscores)
		return respinscores
}